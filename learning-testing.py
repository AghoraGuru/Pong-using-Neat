...
class PongGame:
    ...
    def test_ai(self, genome, config):          #initializing the training Part
        ...
        while run:
            ...
            decision = output.index(max(output))                #gives maximum point of output2

            if decision == 0:                                 #stays still if pong is straight
                pass
            elif decision == 1:
...
if __name__ == "__main__":
    ...
    config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         ...
                         config_path)
    # run_neat(config)          #uncomment this if u wanna make it learn some x generations
    test_ai(config)             #run with this being uncommented and run being commented to play
